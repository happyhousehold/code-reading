function [x  Out] = yall1_ext(A, b, opts)
%
...
% solve the problem
if ~isfield(opts,'x0'); opts.x0 = []; else opts.x0=opts.x0/opts.bmax0; end
if ~isfield(opts,'y0'); opts.y0 = []; end
if ~isfield(opts,'z0'); opts.z0 = []; end
bmax = norm(b,inf); 
t0 = cputime; 
[x,Out] = yall1_gen(A, At, b/bmax, opts.x0, opts.y0, opts.z0, opts);
...
elseif isstruct(A0);
    ...
    error('A must be a matrix or a struct');
end
...
%%%%                   solver                %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [x, Out] = yall1_gen(A,At,b,x0,y0,z0,opts)
% yall1_gen version beta-4 (May 21, 2009)
% Copyright(c) 2009 Yin Zhang
% Minor change by Wotao Yin

%fprintf('--- YALL1 v. b4 ---\n')
%% initialization
m = length(b); sqrtm = sqrt(m); bnrm = norm(b);
[tol,mu,maxit,print,nu,rho,w,nonneg,nonorth] = get_opts;
x = x0; y = y0; z = z0;
if isempty(x0); x = At(b); end
if isempty(y0); y = zeros(m,1); end
if isempty(z0); z = zeros(size(x)); end
if print; iprint1(0); end; % after rpnrm
...
if nonorth; % for non-orthonormal A
    Aty = At(y);
    AAt = @(y) A(At(y));
    if rho > 0; AAt = @(y) AAt(y) + rdmu*y; end
end
...
Out.y = y;
Out.z = z;
Out.exit = 'Exit normally';
if iter == maxit; Out.exit = 'Exit at maxiter'; end
if print; iprint1(1); end


%% nested functions
    function [tol,mu,maxit,print,nu,rho,w,nonneg,nonorth] = get_opts
        % get or set options
        tol = opts.tol;
        mu = mean(abs(b));
        maxit = 9999;
    ...
    function stop = check_stopping
        ...
        rp = A(x) - b; rpnrm = norm(rp);
        if rho > 0;
            p_feasible = 1;
        else
            p_feasible = rpnrm < tol*bnrm;
        end
        if ~p_feasible; return; end
        %objp = sum(abs(w.*x));
        %objd = b'*y;
        %if rho > 0
        ...
        %if gap_small; stop = 1; end
        
        rel_change=norm(x-xp)/norm(xp); 
        change_small=rel_change<tol;
        if change_small; stop=1; end
    ...
    function iprint2
        ...
        if isfield(opts,'xs') && ~isfield(opts,'nu')
            ...
            Out.optim = [Out.optim max([dgap rdnrm rpnrm])];
