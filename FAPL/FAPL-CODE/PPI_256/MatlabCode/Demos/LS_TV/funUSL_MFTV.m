function [xout, etc] = funUSL_MFTV(data, control, objA)
...
[f,g,f0] = neworacle(data, p0, control,objA);
% compute t_p0 \in argmin{ f(p_0) + g(p_0)' * (x - p0): \|x\| \le R }
t_p0  = - data.R * g / norm(g);


[ft,gt,ft0] =  neworacle(data, t_p0, control,objA );
...
while (terminate==0)
    ...
    while 1 == 1
        ...
        [f,g,f0]= neworacle(data, x_lbt, control,objA);
       ...
       if (norm(x_t)>data.R || er==1)
        ...
        [fu, fu0] = funOraclef(data, tmp_xub, control,objA);
