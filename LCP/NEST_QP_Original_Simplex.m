...
function NEST_QP_Original_Simplex(control, domain, data, x0)

%%% initialization
start_NEST=clock;
%data.L = poweriteration(data.A, domain.m, domain.n, 1e-6);
data.L = norm(data.A, inf);
L = 2*data.L;
...
while  t <= control.iter_limit
       ...
       d = 1 + log(zk+(1e-10));       
       hatxk = ProxmappingSimplex(data, domain, 2* sigma * g /(L * (t+3)) - d);
...
str  = sprintf('nstep=%d, UB=%.6e\n,time=%5.2f, L=%5.2f', t, full(UB),time_NEST,L);
disp(str);
