if strcmp(cur_alg, 'RSPG') == 1
    N_description = 'Iteration limit: ';
else
    N_description = 'Number of iterations: ';
end
awr = menu(mtitle, [N_description ,num2str(N_iter)],['Mini-batch size: ',num2str(data.m)] ,['Regularization parameters [lambda1,lambda2,lambda3]: [', num2str(data.lambda1),',',num2str(data.lambda2),',',num2str(data.lambda3),']']  , ['Evaluation sample size (K): ', num2str(N_vali)] , ['Number of runs: ', num2str(Run_times)] , ['Run:', cur_alg], 'Change the algorithm');
if awr == 1
