function [M,mu,A,b,sig,kap,tau,M12] = getM_mu(full,m,n,Ameth,A,b,sig1,sig2,alpha)

% Calculates M and mu, and scales A and b, for each problem type generated
% by getData.m, as per the recommendations given in "A numerical study of
% fixed-point continuation applied to compressed sensing" by Hale, Yin and
% Zhang.  
%
% INPUTS
%
%  full  - only relevant if sig1 > 0 and AA' is not a multiple of I.  In 
%          this case and full == true, M = (sig1^2 AA' + sig2^2 I)^{-1} is 
%          calculated.  Otherwise M is estimated to be (sig1^2 lam_max(AA') 
%          + sig2^2)^{-1} I, the constant is moved into mu, and M is
%          returned as [] (I).
%
...
%          Codes -1, 1, and 3 are equivalent; 2 and 6 are equivalent; 4
...
%          3*sig is recommended tolerance for distinguising between zeros
%          and non-zeros in debias.m.
% 
...
%          should be used.
%
%  M12   - equal to M^(1/2) if ~isempty(M).  user has option to replace A
%          and b with M12*A and M12*b.
%
