...
%function [A,b,x,tau,sigma,y,options] = construct_bpdn_instance(matrix,entries,lambda,options)
% Input:
%  matrix: Defines the model for the matrix. Note that some models are only available in some sizes. Can be any of the following:
...
%     'heaviside':	Gives an nxn spuare matrix with 1/n below the diagonal and zero above.
%     'randorth':	Given a matrix with random orthonormal rows.
%     'union_bases2': Union of two orthonormal bases. Gives [I dctmtx].
%     'union_bases3': Union of three orthonormal bases. Gives [I dctmtx M] with an orthonormalized random matrix.
...
%     'appbernoulli': Gives entries which are generated by sign(randn(s,1)) + 0.1*randn(s,1) (i.e. "normally distributed with standard variation 0.1 around +-1").
%  lambda: A positive number which is the regularization parameter in front of the 1-norm.
%
% Optional input (for some matrix models restricted):
%  options: A struct containing optional input options. Set options = [] to use the defaults.
%  options.dim        [k n] dimension of the matrix A (default [200 1000])
%         .sparsity   s the number of nonzero entries in x (default round(k/8))
%         .seed       Seed for the random number generator (for reproducible results, no default)
%         .scale      1='scale to norm(A)=1', 0='no scaling' (default 1)
%         .verbose    0=quiet, 1=some output (default 0)
%         .solver     Solver for the construction of y ('pocs', 'conditional', 'projected', default 'pocs')
%         .maxiter    Maximal number of iteration for the solver (default: 10*n)
%
% Output:
%  A:	The k times n matrix.
%  b:   The right hand side (a k vector).
%  x:	An n vector which is the minimizer of: 0.5||A*x-b||_2^2 + lambda*||x||_1.
%  tau:	norm(x,1). Useful for tests of (LS_tau).
%  sigma:	norm(A*x-b,2). Useful for tests of (BP_sigma).
%  y:       Another k vector. Can be used the build new right hand sides for different lambda and different x (which has the same sign pattern) by b = lambda*y + A*x.
...
%  [A,b,x,tau,sigma,y] = construct_bpdn_instance('partdct','dynamic5', 0.001,options);
%
