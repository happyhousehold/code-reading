...
%        cvx_end
    ...
    lb_prob.c = g;
    lb_prob.a = [ones(1, domain.n); -sparse(BarX.a')];
    lb_prob.blc = [Bnd;-inf];
    lb_prob.buc = [Bnd;-BarX.b];
...
%            lb_t = cvx_optval;
        ...
        lb_prob.c = g;
        %specify the linear constrain matrix
        lb_prob.a = [ones(1,domain.n); -sparse(BarX.a'); sparse(Bundle.matrix(1:Bundle.size,:))];
        lb_prob.blc = [Bnd; -inf; -inf*ones(Bundle.size,1)];
        lb_prob.buc = [Bnd; -BarX.b; LS * ones(Bundle.size, 1) - Bundle.const(1:Bundle.size)];
        lb_prob.blx = zeros(domain.n,1); 
        lb_prob.bux = [];
    else,
...
%            cvx_end
        ...
        %specify the linear constrain matrix
        lb_prob.a = [sparse([ones(1,domain.n),0]); -sparse([BarX.a',0])]; 
