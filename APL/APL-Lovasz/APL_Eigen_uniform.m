...
function APL_Eigen_uniform(control, domain, data)
 ...
 %   data.mu = 0;
    ...
    while 1 == 1   
        ...
        if stepLB >= LS - control.theta * (LS - LB),
         ...
         %       xt = ProxMappingCVX(domain, Bundle, data, BarX, f, g,
         %       x_lbt, c, LS);
...
str  = sprintf('nstep=%d, UB=%.7e, LB=%.7e, stepUB=%.7e, stepLB=%.7e, Q=%.4e, time=%5.2f\n',...
    nstep, UB, LB, stepUB, stepLB, Q0, time_APL);
disp(str);
