...
while(1==1)
    ...
    if imenu == 1,
        % default settings    
        dataF.domain.d_arc = 100;
        dataF.domain.m = 50;
%        dataF.domain.type = 'sf';
            
        dataF.domain.m = input('number of nodes > ');
        dataF.domain.d_arc = input('designed number of arcs > ');          

        [A,n,exA] = NewLovaszInst(dataF.domain.m, dataF.domain.d_arc);
        dataF.data.exA = exA;
        dataF.data.m = dataF.domain.m;
        dataF.data.n = n;
        dataF.domain.n = n;
...
%            strSubLambda = sprintf('Subproblem lambda: %5.2f', control.SubLambda);
   ...
   if imenu==4,
        ...
        while(1==1)
            ...
            irun=menu(strP, strIter, strEpsilon, strDType, 'Run','->');
            if irun == 5,
                break;
            end;
            if irun == 1,
